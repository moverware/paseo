...
| Prompt images for external turns      | `agent/prompt-images.ts` — a turn's image blocks are written under `<paseo home>/prompt-images/<agent>/` with a manifest; Claude sessions write them for the route hook to append as paths, Codex sessions append the paths themselves to the prompt they hand the pane (the pane only takes text)                                                                                                                                                                          |
| Routed-turn rendering                 | a `⤳`-marked hook refusal renders as one line, or silently                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Blocked-prompt surfacing              | `agent/providers/claude/blocked-prompt-note.ts` — a pane-side `UserPromptSubmit` hook block seen through the tail renders as a timeline note and closes the activity turn, so a swallowed prompt is never silent                                                                                                                                                                                                                                                            |
| Model and effort mirroring            | the external process's model and effort are read from its transcript (effort from the `effort` stamp on assistant entries and `/effort` output) and announced as `model_changed` / `thinking_option_changed`; an effort chosen on the phone goes to the pane as `/effort`. Codex effort comes from each turn's `turn_context` row (`providers/codex/external-rollout.ts`); the Codex TUI has no effort command, so a phone change for a Codex pane is refused with a notice |
| 1M context under pool auth            | `providers/claude/fable-context-suffix.ts` — daemon-spawned children run under API-style auth, where Claude Code assumes 200k for plain Fable ids; the spawned process's model option gets the `[1m]` suffix while config and mirroring keep the canonical id                                                                                                                                                                                                               |
...
| Live activity readout                 | `update_agent_request.externalActivity` (fed by the session-handoff watcher scraping the pane's TUI ticker) is held transiently on the agent and — because the phone runs the official app — projected into the TITLE while the external turn runs, so deep work is distinguishable from a hang                                                                                                                                                                             |
| Stop escalation                       | `AgentSession.forceReleaseHungRuntime` — when an interrupt cannot settle, `cancelAgentRun` kills the provider runtime tree so the turn fails through the ordinary runtime-exit path instead of zombieing (a wedged syscall held a turn 14 minutes past its stop, 2026-09-01)                                                                                                                                                                                                |
| Late tail arm                         | `onStreamThreadStarted` re-arms the transcript tail once a freshly created session reveals its id — at registration a fresh session has no transcript path, so the arm there is a no-op and a phone-born agent adopted into a pane streamed nothing until a reload (2026-09-04)                                                                                                                                                                                             |
| Own-flush dedupe                      | the tailer skips appends for a settle window after a daemon-side run, and the Claude session drops tailed entries whose transcript uuid its own child already streamed — the provider flushes the transcript after the run's final event, and once every agent was tailed (late tail arm) that flush re-emerged as a second copy of each reply plus a phantom external turn that never closed (2026-09-04)                                                                  |
| No idle frame across a routed handoff | finalizeForegroundTurn holds lifecycle running when the session already has an external turn active or one deferred (`expectsExternalContinuation`) — a routed turn's hook block lands after the pane reported its own turn, and the idle frame in between made the phone app re-send the message it had just delivered (2026-09-05)                                                                                                                                        |
| No steer into an external turn        | `steerActiveTurn` refuses while the pane owns the turn, so a steer-mode phone message (the app's default) falls through to a replacement turn that the prompt hook routes into the pane — steering pushed the text into a daemon-side turn on the shared session that the pane never saw, a same-sid fork (2026-09-06, during a pane /compact)                                                                                                                              |
| Continued conversations               | `providers/claude/continuation.ts` — a `continued-in` row in the tailed transcript (Claude Code parked the pane's conversation into a background worker under a new session id) moves the session onto the successor instead of leaving the mirror on a dead file; `followContinuationsOnDisk` does the same on resume. A non-zero external prompt command posts `EXTERNAL_DELIVERY_FAILED` to the timeline (2026-09-18: a duplicate phone row and a refused daemon resume) |
| Pane questions                        | a Codex mid-turn question read from the rollout raises the same card `CodexAsyncQuestions` gives daemon-run sessions; an answer or dismiss on an externally driven agent goes out through the external prompt command as `PASEO_QUESTION` instead of a steer (2026-09-19: questions were invisible on the phone and blocked every delivery)                                                                                                                                 |
...
members the Claude and Codex providers implement. Four upstream files that the
fork used to patch — `lifecycle-command.ts` and its test,
