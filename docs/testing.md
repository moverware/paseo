...
- App Playwright shares one warmed Metro server per run and gives every Playwright worker its own isolated daemon and `PASEO_HOME`. Tests may run concurrently across and within files without exposing one test's projects, agents, terminals, history, or provider configuration to another worker.
