...
Interactive client creation can use `daemon.externalCreateCommand` to launch a native
terminal agent before the first prompt. This optional JSON-stdin command has
`start`, `commit`, and `rollback` operations; start returns the native session ID,
transcript path (empty until a Codex rollout exists), and routing labels. Creation
registers a mirror and sends the initial prompt through the existing external
command. Startup failures propagate to the client instead of falling back to a
daemon-owned turn. MCP children and schedules retain their normal creation path.

An external session may precede its transcript. Its provider advertises pending
transcript discovery; the tailer streams from the beginning when the file appears.
This preserves the first reply without a synthetic initialization prompt.
