...
test("project.remove.request removes an already-empty project", async () => {
  ...
  // FORK: the bootstrap sweep also emits a bare remove for every archived
  // row; the project removal is the one carrying removedProjectId.
  expect(
    filterByType(emitted, "workspace_update").filter(
      (message) => message.payload.kind === "remove" && message.payload.removedProjectId,
    ),
  ).toEqual([
    {
...
test("workspace recovery stays accepted when git observer warming fails", async () => {
  ...
  // FORK: the bootstrap sweep emits a remove for the still-archived row
  // first; the restore's upsert follows it.
