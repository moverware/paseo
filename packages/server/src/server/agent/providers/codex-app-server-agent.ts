...
export class CodexAppServerAgentSession implements AgentSession {
  ...
  constructor(
    ...
    if (this.resumeHandle?.sessionId) {
      ...
      this.restoreExternalTranscript(this.resumeHandle.metadata);
    }
  }

  private restoreExternalTranscript(metadata: Record<string, unknown> | undefined): void {
    const transcriptPath = metadata?.externalTranscriptPath;
  ...
  async connect(): Promise<void> {
    ...
    // FORK: a session bound to a pane's rollout has no history to read until
    // that rollout exists on disk; the tail streams it from then on.
  ...
  private async establishConnection(): Promise<void> {
    ...
    try {
      ...
      if (this.currentThreadId) {
        // FORK: a pane holds the thread's writer; resuming here would fail
        // with the active-writer error, and thread/read needs no writer.
        if (!this.boundExternalSession) {
  ...
  async getRuntimeInfo(): Promise<AgentRuntimeInfo> {
    ...
    // FORK: a pane-bound session never spawns an app-server of its own.
    if (!this.boundExternalSession) {
      if (this.connectionState === "disconnected") {
        await this.connect();
      }
  ...
  // Returns true when the interrupt was routed externally.
  private interruptExternalTurn(): boolean {
  ...
  // FORK: the external-turn check pushes this one over the complexity limit.
  // eslint-disable-next-line complexity
  async interrupt(): Promise<void> {
    if (this.interruptExternalTurn()) {
      return;
    }
