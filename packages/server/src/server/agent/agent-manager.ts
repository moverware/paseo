...
function externalResumeOptions(
  labels: Record<string, string>,
): AgentResumeSessionOptions | undefined {
  // The external process owns the writer for a mirrored session.
  return labels.origin === "herdr" ? { purpose: "history" } : undefined;
}
...
export class AgentManager {
  ...
  /**
   * True while THIS daemon is executing the turn. An external turn also reads
   * as an in-flight run (it is one — an autonomous turn opened on the agent's
   * behalf), so anything that must tell "we are already broadcasting these
   * events" from "another process is producing them" asks this instead of
   * hasInFlightRun.
   */
  private hasDaemonRun(agentId: string): boolean {
    const agent = this.agents.get(agentId);
    if (!agent) {
      return false;
    }
    if (agent.session.isExternalTurnActive?.()) {
      return false;
    }
  ...
  /**
   * Start streaming this agent's external transcript and tell its session who
   * it is, so the fork-owned commands that reach the external process can find
  ...
  releaseExternalTurn(agentId: string): boolean {
    ...
    agent.session.noteExternalTurn?.("superseded");
  ...
  private async createAgentInternal(
    ...
    const agent = await registerWithExternalPane({
      argv: externalArgv,
    ...
    try {
      ...
      session = handle
        ? await client.resumeSession(
            ...
            providerLaunchConfig,
            launchContext,
            externalResumeOptions(existing.labels),
          )
  ...
  private async writeLabels(agentId: string, patch: AgentLabelPatch): Promise<WriteLabelsResult> {
    ...
    if (liveAgent) {
      ...
      liveAgent.session.noteExternalIdentity?.({ agentId: liveAgent.id, labels: liveAgent.labels });
  ...
  async replaceAgentRun(
    ...
    // FORK: replacing an EXTERNAL turn is not this daemon's interrupt to send.
    if (this.releaseExternalTurn(agentId)) {
  ...
  private async registerSession(
    ...
    try {
      ...
      this.armExternalSession(managed);
      return { ...managed };
    } catch (error) {
      if (!registered) {
        await this.closeUnregisteredSession(session);
